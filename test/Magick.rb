...
class Magick_UT < Test::Unit::TestCase
    ...
    def test_tmpnam
      ...
      assert_equal(tmpfiles+1, Magick._tmpnam_)

      info.texture = texture
      assert_equal(tmpfiles+2, Magick._tmpnam_)

      mon = Magick::ImageList::Montage.new
      mon.texture = texture
      assert_equal(tmpfiles+3, Magick._tmpnam_)

      mon.texture = texture
      assert_equal(tmpfiles+4, Magick._tmpnam_)

      gc = Magick::Draw.new
      gc.composite(0, 0, 20, 20, texture)
      assert_equal(tmpfiles+5, Magick._tmpnam_)

      gc.composite(0, 0, 20, 20, texture)
      assert_equal(tmpfiles+6, Magick._tmpnam_)
